...
def compress_to_file(model: EncodecModel, wav: torch.Tensor, fo: tp.IO[bytes],
    ...
    # Apply bending using the model's apply_bending method
    if model.bending_location == 'between':
        frames = [(model.apply_bending(code).int(), scale) for code, scale in frames]
